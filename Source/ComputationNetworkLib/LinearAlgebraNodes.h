    ...
    virtual void /*ComputationNode::*/ ForwardProp(const FrameRange& fr) override
    {
        ...
        if (!fr.IsOneColumnWrt(inputMBLayout))
        {
            ...
            if (fr.IsBatchMatmul(inputMBLayout) && fr.IsBatchMatmul(inputMBLayout2) && rankA >= 2 && rankB >= 2)
            {
                ...
                Matrix<ElemType> input0 = InputRef(0).ValueFor(fr);
                Matrix<ElemType> input1 = InputRef(1).ValueFor(fr);
                size_t m = m_transpose ? InputRef(0).GetSampleLayout().GetDim(rankA-1) : InputRef(0).GetSampleLayout().GetDim(0);
                size_t n = InputRef(1).GetSampleLayout().GetDim(rankB-1);
        ...
        if (!fr.IsOneColumnWrt(InputRef(0).GetMBLayout()))
        {
            ...
            else if (inputIndex == 1)
            {
                ...
                if (fr.IsBatchMatmul(InputRef(0).GetMBLayout()) &&  fr.IsBatchMatmul(InputRef(1).GetMBLayout()) && rankA >= 2 && rankB >= 2)
                {
                    ...
                    Matrix<ElemType> outputGradient = GradientFor(fr);
                    size_t m = m_transpose ? InputRef(0).GetSampleLayout().GetDim(0) : InputRef(0).GetSampleLayout().GetDim(rankA-1);
                    size_t n = GetSampleLayout().GetDim(rankB-1);
